...
/**
 * Enregistre le plugin DrawSVG afin de pouvoir
 * animer le tracé des éléments SVG avec GSAP.
 */
...
/**
 * Zone de texte de présentation.
 * @type {HTMLTextAreaElement}
 */
const bio = document.querySelector("#bio");

/**
 * Ensemble des boutons radio du rythme de travail.
 * @type {NodeListOf<HTMLInputElement>}
 */
const radios = document.querySelectorAll("input[name='rythme']");

/**
 * Ensemble des cases à cocher des centres d'intérêt.
 * @type {NodeListOf<HTMLInputElement>}
 */
...
/**
 * Conteneur qui affichera la carte profil.
 * @type {HTMLElement}
 */
const profileContainer = document.querySelector("#profileContainer");

/**
 * Carte principale utilisée pour les animations 3D.
 * @type {HTMLElement}
 */
...
/**
 * Zone d'aperçu des données saisies.
 * @type {HTMLElement}
 */
const preview = document.querySelector("#previewData");

/**
 * Bouton permettant de revenir modifier les informations.
 * @type {HTMLButtonElement}
 */
const editBtn = document.querySelector("#editBtn");

/**
 * Bouton permettant de confirmer définitivement.
 * @type {HTMLButtonElement}
 */
...
/**
 * Conteneur affiché après validation finale.
 * @type {HTMLElement}
 */
 ...
 * avant confirmation dans la modale.
 *
 * @type {Object|null}
 */
let tempData = null;
...
async function validateBio() {
    ...
    // 🔥 validation IA
    const isValidIA = await validateBioIA(val);

    if (!isValidIA) {
        setError(bio, "Bio non cohérente ou hors contexte");
        return false;
    }
...
async function validateBioIA(text) {

    const prompt = `
Tu es un système de modération et de validation de bio utilisateur.

Ton rôle est de vérifier si une biographie est acceptable pour une plateforme professionnelle.

=================================================
 OBJECTIF
=================================================
Accepter uniquement les biographies :
- honnêtes
- cohérentes
- professionnelles ou personnelles normales
- sans fausses informations évidentes

=================================================
 INTERDIT ABSOLU
=================================================
Refuser si le texte contient :
- vulgarité, insultes ou langage offensant
- contenu sexuel explicite ou suggestif
- haine, discrimination ou violence
- spam, publicité, crypto, arnaques
- texte incohérent ou sans sens
- fausses déclarations évidentes (ex: "je suis président", "je suis milliardaire", sans contexte crédible)
- répétitions inutiles ou caractères aléatoires
...
- phrases courtes ou longues normales
- langage simple et naturel

=================================================
 RÈGLE IMPORTANTE
=================================================
- NE PAS juger la qualité du style
- NE PAS exiger un texte long
- NE PAS refuser une bio juste parce qu'elle est courte
- NE PAS bloquer une auto-présentation simple et réaliste

=================================================
 DÉTECTION DE FAUSSES INFORMATIONS
=================================================
Refuser uniquement si l’affirmation est clairement absurde ou trompeuse sans contexte (ex: "je suis roi du monde", "je suis PDG de Google").
    ...
    try {
        ...
        const res = await fetch(
            "https://openrouter.ai/api/v1/chat/completions",
            {
                method: "POST",
                headers: {
                    "Authorization": "Bearer ${OPENROUTER_API_KEY}", // ta clé ici
        ...
        console.log("BIO IA RESULT:", result);

        return result === "valid";

    } catch (error) {
        console.error("Erreur IA bio:", error);

        // fallback : on accepte pour ne pas bloquer l'app
        return true;
    }
}
...
form.addEventListener("submit", async (e) => {
    ...
    const isValid =
        await validateName(firstName, "Prénom") &
        await validateName(lastName, "Nom") &
        await validateEmail() &
